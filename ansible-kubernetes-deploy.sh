...
kubectl apply -f k8s_deployments/deployment.yml --validate=false || { echo "Failed to apply Kubernetes deployment! Exiting."; exit 1; }
kubectl apply -f k8s_deployments/service.yaml --validate=false || { echo "Failed to apply Kubernetes service! Exiting."; exit 1; }
